...
folder1 = "/rc3_part_lookup_serp_morb_bas_h2o_1denslookup_0visclookup_cohes_1-MPa_freesurf_y_extent_1450km_sed_1km_smu0_02_cmu0_04_deserp_erase_res5_5_run41"
ids1 = ["02000"]
...
folder3 = "/rc3_part_lookup_serp_morb_bas_h2o_1denslookup_0visclookup_cohes_1-MPa_freesurf_y_extent_1450km_sed_1km_smu0_02_cmu0_04_deserp_erase_res5_5_run38_new"
ids3 = ["02000"]
files3 = [os.getcwd()+folder3+"/solution/solution-"+id3+".npz" for id3 in ids3]

folder4 = "/rc3_part_lookup_serp_morb_bas_h2o_0denslookup_0visclookup_cohes_1-MPa_freesurf_y_extent_1450km_sed_1km_smu0_02_cmu0_04_deserp_erase_res5_5_run12_new"
ids4 = ["02000"]
...
colors = ["slategrey","skyblue","sienna","indigo"]
        ...
        for kin_file in kin_files:
            ...
            if fidx>-1:
                if plot_time_xaxis:
                    distance_vc = t[1:]
                    axM1.set_xlim([0,50])
                    axM1.set_xlabel("Time [Myr]",fontdict=font)
                else:
                    distance_vc = np.cumsum(1e6*np.diff(t)*vc[1:]*0.01/1e3)
                    axM1.set_xlim([0,1640])
                    axM1.set_xlabel("Net convergence [km]",fontdict=font)
                axM1.set_ylabel("v$\mathregular{_{c}}$ [cm/yr]",color="black",fontdict=font) # np.cumsum(1e6*np.diff(t)*vc[1:]*0.01/1e3)

                axM1.plot(distance_vc,vc[1:],color=colors[fidx],linewidth=2.0)

                
                axM1.plot(sum(1e6*np.diff(t[:11])*vc[1:11]*0.01/1e3),vc[10],marker="*",markerfacecolor=colors[fidx],markeredgecolor="black",markersize=16,zorder=10)
...
if plot_time_xaxis:
    boundwater_name = "/S8_time"
else:
    boundwater_name = "/S8_convergence"
