...
def main():
    ...
    output_file = files1[0].replace(".npz", "_fig6_outline.png")
...
def plot_col(fig,ax,sub_gs,files,row,col,ids,folder):
    ...
    for idx,file in enumerate(files):
        ...
        ax_comp.tricontourf(triangulation, freefluid,levels=[4e-5,1.1],colors=["blue",],alpha=0.45,zorder=2)
        ...
        vel_vects = ax_comp.quiver(x_grid, y_grid, 
                        ...
                        color='black',scale=50,width=0.003,zorder=2) # scale=150, width=0.0015
        ...
        if is_plot_outline:
            ...
            if (2*row+col)==0:
                title_name = " $\mathregular{\mu_{serp}}$=0.01"
            elif (2*row+col)==1:
                title_name = " $\mathregular{\mu_{serp}}$=0.02"
            elif (2*row+col)==2:
                title_name = " $\mathregular{\mu_{serp}}$=0.03"
            elif (2*row+col)==3:
                title_name = " $\mathregular{\mu_{serp}}$=0.04"
